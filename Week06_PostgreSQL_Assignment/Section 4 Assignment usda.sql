...
=> elect * from data_src WHERE year = 1990 AND journal = 'Cereal Foods World';
...
=> select COUNT(*) from nutr_def WHERE units='kj' OR units='kcal';

11. Select all records from data source table (data_src) that where from the year 2000 or the journal Food Chemistry.
=> select COUNT(*) from data_src WHERE year=2000 or journal='Food Chemistry';
...
--15. Select all records from the data source table that were published 
